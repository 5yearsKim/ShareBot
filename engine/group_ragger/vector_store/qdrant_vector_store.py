...
class QdrantVectorStore(BaseVectorStore):
    def __init__(
        self, qdrant_url: str = "localhost:6333", namespace: str = "test"
    ) -> None:
    ...
    def get(self, id: int | str) -> Point:
        retrieved: list[Any] = self.client.retrieve(
            collection_name=self.namespace, ids=[id]
        )
    ...
    def get_many(
        ...
        fetched, next_cursor = self.client.scroll(
            ...
            scroll_filter=Filter(must=must_filter),
    ...
    def upsert(self, point: Point) -> None:
        self.client.upsert(collection_name=self.namespace, points=[point.to_qdrant()])
    ...
    def delete_many(self, ids: list[int | str]) -> None:
        self.client.delete(
            collection_name=self.namespace, points_selector=PointIdsList(points=ids)
        )
