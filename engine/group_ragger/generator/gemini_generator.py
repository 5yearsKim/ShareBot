...
class GeminiGenerator(BaseGenerator):
    ...
    def _convert_messages_to_prompt(self, messages: list[Message]) -> str:
        prompt = (
            "너는 아래 대화에서 bot 이야. 다음 대화에 이어질 적절한 답변을 해줘.\n\n"
        )
    ...
    def generate_stream(self, messages: list[Message]) -> Iterable[StreamOutput]:
        ...
        response_stream = self.model.generate_content(
            prompt, safety_settings={"HARASSMENT": "block_none"}, stream=True
        )
        ...
        def response_streamer() -> Iterable[StreamOutput]:
            ...
            for chunk in response_stream:
                text += chunk.text
                yield StreamOutput(chunk=chunk.text, text=text, status="progress")
            # end streamin
