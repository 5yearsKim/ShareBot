...
from ..schema import Message, MessageRole, StreamOutput
...
class ClaudeGenerator(BaseGenerator):
    ...
    def generate_stream(self, messages: list[Message]) -> Iterable[StreamOutput]:
        ...
        with self.client.messages.stream(
            ...
            messages=[m.to_claude() for m in claude_msgs],  # type: ignore
        ) as stream_response:
            for chunk in stream_response.text_stream:
                text += chunk
                yield StreamOutput(chunk=chunk, text=text, status="progress")
            yield StreamOutput(chunk="", text=text, status="done")
